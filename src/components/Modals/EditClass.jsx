...
const EditClassModal = ({ open, handleCancel, handleOk, modalData }) => {
  ...
  const [require_moderator_approval, setRequireModeratorApproval] = useState(
    modalData?.require_moderator_approval == "1"
  );
  const [all_users_join_as_moderator, setAllUsersJoinAsModerator] = useState(
    modalData?.all_users_join_as_moderator == "1"
  );
  const [logo_url, setLogoUrl] = useState(modalData?.logo_url);
  const [logout_url, setLogoutUrl] = useState(modalData?.logout_url);
  const [primary_color, setPrimaryColor] = useState(modalData?.primary_color);
  ...
  const [skip_check_audio, setSkipCheckAudio] = useState(
    modalData?.skip_check_audio == "1"
  );
  const [disable_listen_only_mode, setDisableListenOnlyMode] = useState(
    modalData?.disable_listen_only_mode == "1"
  );
  const [enable_user_private_chats, setEnableUserPrivateChats] = useState(
    modalData?.enable_user_private_chats == "1"
  );
  const [class_layout, setClassLayout] = useState(modalData?.class_layout);
                    ...
                    children: (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <option
                              selected={class_layout == "SMART_LAYOUT"}
                              value="SMART_LAYOUT"
                            >
                              Default
                            </option>
                            ...
                            <option
                              selected={class_layout == "PRESENTATION_FOCUS"}
                            ...
                            {/* video focused */}
                            <option
                              selected={class_layout == "VIDEO_FOCUS"}
                              value="VIDEO_FOCUS"
                            >
                              Video Focused
                            </option>
